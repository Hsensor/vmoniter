...
module.exports = function($scope,$q,diskColdhotlayerSrv,$timeout) {
  $scope.periods = [
    {
      period:60,text:"1分钟"
    },
  ...
  function listDisks(baseOptions){
    ...
    diskColdhotlayerSrv.listDisks().then(function(result){
      var HDD_disks = [];
      var SSD_disks = [];
      ...
      for(var i=0;i<result.entities.length;i++){
        ...
        if(result.entities[i].storageTierName=="HDD"){
          ...
          HDD_disks.push(diskColdhotlayerSrv.GetMetricStatistics(
            {
              "version":"2010-08-01",
              "namespace":"vega",
          ...
          SSD_disks.push(diskColdhotlayerSrv.GetMetricStatistics(
            {
              "version":"2010-08-01",
              "namespace":"vega",
              "statistics":tempOptions.statistics,
      ...
      $q.all(result.hdd).then(function(result){
        drawChart({type:"HDDChart",lines:result});
      });
  ...
  function drawChart(lineObj){
    ...
    for(var i=0;i<lineObj.lines.length;i++){
      var temp = {name:series[i].cvmIpAddress+"-"+series[i].mountPath,data:[],color:colors[i]};
      lines.push(temp);
        ...
        formatter: function () {
            ...
            this.points.forEach(function(item){
              s += '<br/><tspan style="fill:'+item.series.color+'" x="8" dy="15">●</tspan><tspan>'+ item.series.name + ': ' +
                    chart.conversion(item.y,{decimalCount:2})+"</tspan>";
            })
