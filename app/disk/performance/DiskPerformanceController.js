...
module.exports = function($scope,$q,diskPerformanceSrv,$timeout) {
  $scope.periods = [
    {
      period:60,text:"1分钟"
    },
  ...
  function listDisks(baseOptions){
    ...
    diskPerformanceSrv.listDisks().then(function(result){
      var HDD_disks = [];
      var SSD_disks = [];
      ...
      for(var i=0;i<result.entities.length;i++){
        ...
        if(result.entities[i].storageTierName=="HDD"){
          ...
          HDD_disks.push(diskPerformanceSrv.GetMetricStatistics(
            {
              "version":"2010-08-01",
              "namespace":"vega",
          ...
          SSD_disks.push(diskPerformanceSrv.GetMetricStatistics(
            {
              "version":"2010-08-01",
              "namespace":"vega",
              "statistics":tempOptions.statistics,
  ...
  function drawChart(lineObj){
    ...
    for(var i=0;i<lineObj.lines.length;i++){
      ...
      for(var j=0;j<lineObj.lines[i].datapoints.length;j++){
        temp.data.push(Math.ceil(lineObj.lines[i].datapoints[j][1]/series[i].diskSize*10000)/100);
      }
    }
    ...
    chart.initChartOption($scope,{name:lineObj.type},chart.chartOptions,function(scopeChart){
        ...
        formatter: function () {
            ...
            this.points.forEach(function(item,index){
              s += '<br/><tspan style="fill:'+item.series.color+'" x="8" dy="15">●'+ item.series.name + ': ' +
                    chart.conversion(item.y*series[index].diskSize/100,{decimalCount:2})+"/"+chart.conversion(series[index].diskSize,{decimalCount:2})+"("+item.y+"%)</tspan>";
            })
            return s;
        },
        shared: true
      ...
      scopeChart.yAxis.min = 0;
      scopeChart.yAxis.max = 100;
