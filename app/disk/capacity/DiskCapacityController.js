  ...
  function drawChart(lineObj){
    ...
    for(var i=0;i<lineObj.lines.length;i++){
      ...
      for(var j=0;j<lineObj.lines[i].datapoints.length;j++){
        temp.data.push(Math.ceil(lineObj.lines[i].datapoints[j][1]/lineObj.disks[i].diskSize*10000)/100);
      }
    }
        ...
        formatter: function () {
            ...
            this.points.forEach(function(item,index){
              s += '<br/><tspan x="8" dy="15" style="fill:'+item.series.color+'">●'+ item.series.name + ' ' +
                    chart.conversion(item.y*lineObj.disks[index].diskSize/100,{decimalCount:2})+"/"+chart.conversion(lineObj.disks[index].diskSize,{decimalCount:2})+"("+item.y+"%)</tspan>";
            })
